...
parser.add_argument(
    "--nr_trials", type=int, default=10, required=False, help="Number of splits"
)
...
nr_nontraining_images = int(
    nr_images * (args.test_percentage + args.val_percentage) * 0.01 + 0.5
)
...
for i in range(args.nr_trials):
    ...
    ann_train_out_path = (
        args.dataset_dir + "/" + "annotations_" + str(i) + "_train.json"
    )
